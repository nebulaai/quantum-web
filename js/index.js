...
function start_app() {
    ...
    nebula.initialize()
        ...
        .then((is_submissible) => {
            if (is_submissible) {
                //load default app.
                app_ready().then(console.log).catch(console.log);
            } else {
                //load current task. todo temp testing
                resume_task().then(console.log).catch(console.log);
            }
        });
}
...
const payToken = function () {
    nebula.get_client_info()
        .then(result => {
            console.log("Checking client information");
            if(result[0]) {
                console.log("Client has been banned");
                throw "Client banned"
            }
        ...
        .then((is_submissible) => {
            if (is_submissible) {
                nebula.current_task["name"] = $("#taskName").val();
                // nebula.current_task["script"] = "script 2";
                // nebula.current_task["data"] = "data 1";
                // nebula.current_task["output"] = "output 1";
                nebula.current_task["params"] = JSON.stringify({
                    "epoch": $("#epoch").val()
                });
                nebula.current_task["fee"] = web3.toWei(
                    parseFloat($("#tx_fee_value").val()),
                    "ether"
                );
                return nebula.create_task();
            } else {
                //todo
                throw "Client cannot submit more task";
            }
        })
        .then((result) => {
